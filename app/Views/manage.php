...
<body>
   ...
   <div class="container-fluid mt-4">
      ...
      <div class="row">
         <div class="col-12">
           <p class="breadcume">Crisis Sim > Manage Exercise</p>
         </div>
      </div>

<div class="row">
   <div class="col-12">
      <h2 class="main-head">Manage Exercise</h2>
   </div>
</div>
                        ...
                        <?php if (isset($industory_nam)){?>
                           <?php foreach($industory_nam as $key){?>
                                 <div class="d-flex">
                                    ...
                                    $db=\Config\Database::connect();
                                    $max_no=$db->query("SELECT COUNT(*) as max_non  FROM imr_single_player_setup usb left join imr_crisis_scenrio ics on usb.crisis_scenario_id=ics.id WHERE usb.published_status=1 and ics.industry_sector =".$key["id"]." AND usb.created_by=$selected_scen ORDER BY usb.id DESC")->getRow();
                     ...
                     <div class="nav-one">
                           ...
                           <?php foreach($attack_nam as $key){?>
                                 ...
                                 <input type="checkbox" name="attack" class="common_select attack" style="padding: 0.3rem;" value="<?= $key["id"];?>">
                                    ...
                                    $db=\Config\Database::connect();
                                    $max_no=$db->query("SELECT COUNT(*) as max_non  FROM imr_single_player_setup usb left join imr_crisis_scenrio ics on usb.crisis_scenario_id=ics.id WHERE usb.published_status=1 and ics.attack_vector =".$key["id"]." AND usb.created_by=$selected_scen ORDER BY usb.id DESC")->getRow();
                                    // var_dump($max_no->max_non);die;
                                    echo " (".$max_no->max_non.")";
                                    ?></p>
                                 </div>
                        ...
                        <div class="slide" style="">
                           ...
                           <?php foreach($threat_nam as $key){?>
                                 ...
                                 <input type="checkbox" name="threat" class="common_select threat" style="padding: 0.3rem;" value="<?= $key["id"];?>">
                                    ...
                                    $db=\Config\Database::connect();
                                    $max_no=$db->query("SELECT COUNT(*) as max_non  FROM imr_single_player_setup usb left join imr_crisis_scenrio ics on usb.crisis_scenario_id=ics.id WHERE usb.published_status=1 and ics.threat_actor =".$key["id"]." AND usb.created_by=$selected_scen ORDER BY usb.id DESC")->getRow();
                                    // var_dump($max_no->max_non);die;
                                    echo " (".$max_no->max_non.")";
                                    ?></p>
                                 </div>
      ...
      $(document).ready(function(){
         ...
         $(".common_select").click(function(){
            ...
            $.ajax({
               ...
               async:false,
               dataType:"json",
               url:"<?=base_url()?>/Manage/getfilterscenario",
