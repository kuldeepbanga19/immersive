...
<body>
   ...
   <div class="container-fluid mt-4">
      ...
      <div class="row">
         <div class="col-12">
           <p class="breadcume">Crisis Sim > Result</p>
         </div>
      </div>

<div class="row">
   <div class="col-12">
      <h2 class="main-head">Result</h2>
   </div>
</div>
...
<ul class="nav nav-tabs" id="myTab" role="tablist">
   <li class="nav-item" role="presentation">
     <button class="nav-link active" id="home-tab" data-bs-toggle="tab" data-bs-target="#home" type="button" role="tab" aria-controls="home" aria-selected="true">Your Result</button>
   </li>
   <!-- <li class="nav-item" role="presentation">
     <button class="nav-link" id="profile-tab" data-bs-toggle="tab" data-bs-target="#profile" type="button" role="tab" aria-controls="profile" aria-selected="false">Organization Result</button>
   </li> -->
 </ul>
            ...
            <div class="navigation">
               ...
               <nav>
                  <ul class="ul p-0 mb-0">
                        ...
                        <div class="slide">
                           ...
                           <?php foreach($industory_nam as $key){?>
                                 <div class="d-flex">
                                    ...
                                    $max_no=$db->query("SELECT COUNT(*) as max_non FROM  imr_single_player_setup usb left join imr_crisis_scenrio ics on ics.id=usb.crisis_scenario_id  WHERE usb.id  IN (SELECT usb.id FROM imr_single_player_setup usb LEFT JOIN imr_player_assignment imp ON usb.id=imp.singlep_id  WHERE usb.published_status=1 AND imp.userid=$selected_scen AND imp.status=1  ORDER BY usb.id DESC) and industry_sector=".$key["id"]."")->getRow();

                                    // $max_no=$db->query("SELECT COUNT(*) as max_non FROM yatharth_immersive.imr_single_player_setup usb inner JOIN imr_player_assignment imp ON usb.id=imp.singlep_id  join yatharth_immersive.imr_crisis_scenrio cse on usb.crisis_scenario_id=cse.id where cse.publish_status=1 and cse.industry_sector =".$key["id"]." AND usb.published_status=1 AND (imp.userid=$selected_scen) AND imp.status=1  ORDER BY usb.id DESC")->getRow();
                     ...
                     <div class="nav-one">
                           ...
                           <?php foreach($attack_nam as $key){?>
                                 ...
                                 <input type="checkbox" name="attack" class="common_select attack" style="padding: 0.3rem;" value="<?= $key["id"];?>">
                                    ...
                                    // $max_no=$db->query("SELECT COUNT(*) as max_non FROM yatharth_immersive.imr_single_player_setup usb inner JOIN imr_player_assignment imp ON usb.id=imp.singlep_id  join yatharth_immersive.imr_crisis_scenrio cse on usb.crisis_scenario_id=cse.id where cse.publish_status=1 and cse.attack_vector =".$key["id"]." AND usb.published_status=1 AND (imp.userid=$selected_scen) AND imp.status=1  ORDER BY usb.id DESC")->getRow();
                                    // var_dump($max_no->max_non);die;
                                    echo " (".$max_no->max_non.")";
                                    ?></p>
                                 </div>
                     ...
                     <div class="nav-one">
                        <a href="#" style="text-decoration: none; color: #000;">
                           <li class="py-2 d-flex justify-content-between align-items-center"><b>Threat Actor</b><i
                                 class="fa fa-caret-down" aria-hidden="true"></i></li>
                        </a>
                        <div class="slide" style="">
                           ...
                           <?php foreach($threat_nam as $key){?>
                                 ...
                                 <input type="checkbox" name="threat" class="common_select threat" style="padding: 0.3rem;" value="<?= $key["id"];?>">
                                    ...
                                    // $max_no=$db->query("SELECT COUNT(*) as max_non FROM yatharth_immersive.imr_single_player_setup usb inner JOIN imr_player_assignment imp ON usb.id=imp.singlep_id  join yatharth_immersive.imr_crisis_scenrio cse on usb.crisis_scenario_id=cse.id where cse.publish_status=1 and cse.threat_actor =".$key["id"]." AND usb.published_status=1 AND (imp.userid=$selected_scen) AND imp.status=1  ORDER BY usb.id DESC")->getRow();
                                    // var_dump($max_no->max_non);die;
                                    echo " (".$max_no->max_non.")";
                                    ?></p>
                                 </div>
                                    ...
                                    <div class="flx mt-4">
                                       <div class="inprogress">
                                         ...
                                         <?php  if($value['set_up_type']==3){?>
                                          <a href="<?= base_url('/Presentation_player_setup/published_exercise/'.$value['singlep_id'])?>" class="underline"> 
                                              <?php 
                                          ...
                                          <a href="<?= base_url('/Player_result/player_result/'.$value['singlep_id'])?>" class="underline"> 
                                              <?php 
                                          if($value['start_status']==1){?>
      ...
      $(document).ready(function(){
         ...
         $(".common_select").click(function(){
            ...
            $.ajax({
               ...
               async:false,
               dataType:"json",
               url:"<?=base_url()?>/Result_main/getfilterscenario",
