...
width: 40%;
   ...
   <li data-qaid="stepsProgressDetailStep-3" class="li">
      ...
      <script>
      	$(".save-next").click(function(){
           ...
           var scenid=$("#scen_id").val(); 
      		...
      		$.ajax({
      			...
      			url:"<?=base_url()?>/Single_player_setup/addinfo",
      			success:function(json){
      				if(json.success){
      					// alert("hellooo");
                var spid=json.sp_id;
                window.location.href="<?=base_url()?>/Single_player_setup/addusers/"+spid+'/'+scenid;
      				}
      			}
      		})
      	})
      	$(".bck").click(function(){
      		var scenid=$("#scen_id").val();
      		window.location.href="<?=base_url()?>/Single_player_setup/single_player/"+scenid;
      	})
