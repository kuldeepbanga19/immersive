...
class Scenario_feedback extends BaseController
{
 protected $userid;
...
// view scenario feedback
  public function scenario_feedback($id=0)
  {	
    $session = session();
if(!$session->has('us')){
         return redirect()->to('login');	
  ...
  if($data['user_obj']['modified']){
    ...
    $data["scenar_injects"]=$injects;
      ...
      return view('scenario_feedback',$data);
  ...
  public function updatefeedback($id){
    ...
    $SessionModel = new SessionModel();
     ...
     $feedback_performanceDb=new Feedback_performanceDb();
     $data34 = ['crisis_scenario_id'=>$scenario_id,'created' =>strtotime($time),'created_by'=>$usernameID,'feedback_type'=>$feedback_type];
     ...
     echo json_encode($json);
      ...
      $builder->update(['modified' =>strtotime($time),'modified_by'=>$usernameID,'feedback_type'=>$feedback_type] , ['id' => $bid]);
      $json=array();
      $json['success']=true;
     echo json_encode($json);
      }
