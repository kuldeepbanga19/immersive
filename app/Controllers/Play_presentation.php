...
class Play_presentation extends BaseController
{
...
public function play_presentation($spid=null)
{	
// print_r($spid);die;
$session = session();
if(!$session->has('us')){
 ...
 $data['spid']=$spid;
 $single_player_setup = $db->query('SELECT * FROM imr_single_player_setup WHERE id='.$spid.'')->getFirstRow();
 $scenario_id=$single_player_setup->crisis_scenario_id;
 $senariotb=$db->preFixtable("crisis_scenrio");
 $scdata=$db->query("SELECT * FROM $senariotb WHERE id=$scenario_id")->getFirstRow();
 if($scdata){
   $data["scenario_title"]=$scdata->scenrio_title;
        ...
        //   echo view('layout/header2',$data);
          return view('play_presentation',$data);
        }
        else{
            // echo view('layout/header1');
            return view('play_presentation'); 
        }
...
public function start_exercise($spid=null)
{	
...
if(!$session->has('us')){
  ...
  $db = \Config\Database::connect();
        ...
        if ($getrole){
        //   echo view('layout/header2',$data);
          return view('presentation_code',$data);
        }
        else{
            // echo view('layout/header1');
            return view('presentation_code'); 
        }
...
public function enter_exercise($spid=null)
{	
...
if(!$session->has('us')){
  ...
  if($scdata){
    $data["scenario_title"]=$scdata->scenrio_title;
    $data["briefing"]=$scdata->briefing;
    $role_cate=$db->query("SELECT * FROM imr_scenario_role_cate WHERE crisis_scenario_id=$scenario_id")->getResultArray();
    $data['roles']=$role_cate;
    
  }
  $Allroles=$db->query("SELECT * FROM imr_scenario_role")->getResultArray();
  $temp_arry=array();
  ...
  $data["rolesinfo"]=$temp_arry;
        ...
        if ($getrole){
        //   echo view('layout/header2',$data);
          return view('enter_presentation',$data);
        }
        else{
            // echo view('layout/header1');
            return view('enter_presentation'); 
        }
...
public function start_play($spid=null)
{	
...
$data['spid']=$spid;
 ...
 if($scdata){
  ...
  $injectsscreated=$db->query("SELECT  injd.*,rl.job_title as jtitle,inj.order_id,inj.timecreated FROM $injectTb inj LEFT JOIN $injdTb injd ON inj.id=injd.inject_id LEFT JOIN $roleTb rl ON rl.id=injd.role WHERE inj.scenario_id=$scenario_id ORDER BY inj.order_id")->getFirstRow();
  
  $data["injectstest"]=$db->query("SELECT  injd.*,rl.job_title as jtitle,inj.order_id,inj.timecreated FROM $injectTb inj LEFT JOIN $injdTb injd ON inj.id=injd.inject_id LEFT JOIN $roleTb rl ON rl.id=injd.role WHERE inj.scenario_id=$scenario_id ORDER BY inj.order_id")->getResultArray();
 //  print_r($injectstest[0]["file"]);die;
  ...
  if($injectsscreated){
     ...
     $pmf_indicators=$db->query("SELECT * FROM $performanceindicatorTb WHERE crisis_scenario_id=$scenario_id")->getResultArray();
...
// print_r($data);die;
        ...
        if ($getrole){
        //   echo view('layout/header2',$data);
          return view('presentation_play_start',$data);
        }
        else{
            // echo view('layout/header1');
            return view('presentation_play_start'); 
        }
...
public function previewData(){
    ...
    if($nextinject){
      ...
      $timecreated=strtoupper(date("H:i__j F Y",(int)$nextinject->timecreated));
        ...
        $optionhtml='<div class="inner-left">
        <p class="main-head mb-0">Your Options</p>
     </div><button class="blue-btn mb-4" id="vote-btn">Vote Now</button><br><p style="color:#fff;">choose your response</p>';
        $itr=1;
        foreach($injectsoptions as $ijop){
        $optionhtml.='<div class="opt_v">
                     <p style="color:#fff;">option'.$itr.'<p>
                     <input type="radio" class="optinput style="color:#fff;"" name="optinput" value="'.$ijop['id'].'"><span style="color:#fff;">'.$ijop['option_title'].'</span>
					<p style="color:#fff;">'.$ijop['option_description'].'<p>	
				    </div>	
                     ';
              $itr++;
         }    
