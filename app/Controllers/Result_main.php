...
class Result_main extends BaseController
{
  ...
  $data['threat_nam']=$db->query('SELECT * FROM yatharth_immersive.imr_threat_actor')->getResultArray();
      ...
      if ($db->query('SELECT * FROM yatharth_immersive.imr_user_role WHERE role_id=1 AND user_id='. $usernameID)->getResult()){
        $tempvv = $db->query('SELECT * FROM yatharth_immersive.imr_user WHERE id='. $usernameID)->getFirstRow();
            ...
            //   $presentation_data=$db->query("SELECT usb.*,imp.* FROM $usertb usb LEFT JOIN imr_player_assignment imp ON usb.id=imp.singlep_id WHERE usb.published_status=1 AND (imp.userid=$usernameID) AND imp.status=1 OR usb.set_up_type=3 ORDER BY usb.id DESC")->getResultArray();


            $presentation_data=$db->query("SELECT usb.*,imp.* FROM $usertb usb LEFT JOIN imr_player_assignment imp ON usb.id=imp.singlep_id WHERE usb.published_status=1 AND (imp.userid=$usernameID) AND imp.status=1 ORDER BY usb.id DESC")->getResultArray();
      ...
      elseif($db->query('SELECT * FROM yatharth_immersive.imr_user_role WHERE ( role_id=2 or role_id=3) AND user_id='. $usernameID)->getResult()){
        ...
        $data["firstletter"]=$fstl[0];
        $data["lastletter"]=$lstl[0];
        // piyusha for persentation(03 DEC)
        // $presentation_data=$db->query("SELECT usb.*,imp.* FROM $usertb usb LEFT JOIN imr_player_assignment imp ON usb.id=imp.singlep_id WHERE usb.published_status=1 AND (imp.userid=$usernameID) AND imp.status=1 OR usb.set_up_type=3 ORDER BY usb.id DESC")->getResultArray();
        // end
      ...
      $presentation_data=$db->query("SELECT  usb.* FROM  imr_single_player_setup usb left join imr_crisis_scenrio ics on ics.id=usb.crisis_scenario_id  WHERE usb.id  IN (SELECT usb.id FROM imr_single_player_setup usb LEFT JOIN imr_player_assignment imp ON usb.id=imp.singlep_id  WHERE usb.published_status=1 AND imp.userid=$data_array AND imp.status=1  ORDER BY usb.id DESC) $indusrtrywhere $attackwhere $threatwhere $searchwhere")->getResultArray();

        
      // $presentation_data=$db->query("SELECT usb.*,imp.* FROM imr_single_player_setup usb inner JOIN imr_player_assignment imp ON usb.id=imp.singlep_id join imr_crisis_scenrio cse on usb.crisis_scenario_id=cse.id where  usb.published_status=1 $indusrtrywhere $attackwhere $threatwhere $searchwhere AND (imp.userid=$data_array) AND imp.status=1  ORDER BY usb.id DESC")->getResultArray();
      ...
      // }
        ...
        $threat_actorDb = new Threat_actorDb();
        $data['ThreatData'] = $threat_actorDb->orderBy('LTRIM(threat_name)')->findAll();
        $json["filterddata"]=view("filterresult_main",$data);
        $json["success"]=true;
